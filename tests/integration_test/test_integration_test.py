...
# if you change the port number, make sure to change them in the html
# pages of mock retailers as well
...
def test_push_to_db_no_today_data_tables(
    ...
    df_to_replace_dict = {
        "lowest_prices_tiered": df_dict_to_push["lowest_prices_tiered"]
    }
    ...
    obtained_gpu_of_interest_df = df_dict_to_push["gpu_of_interest"].sort_values(
        by="gpu_price", ignore_index=True
    )
    obtained_lowest_prices_df = df_dict_to_push["lowest_prices"].sort_values(
        by="gpu_price", ignore_index=True
    )
    obtained_lowest_prices_tiered_df = df_dict_to_push[
    ...
    pdt.assert_frame_equal(
        obtained_lowest_prices_tiered_df,
        expected_lowest_prices_tiered_df,
        rtol=0.001,
        atol=0.001,  # for us, max tolerance could be 0.01
    )

    pdt.assert_frame_equal(
        left=pd.read_sql(
            sql="SELECT * FROM gpu_of_interest", con=test_db_conn
        ).sort_values(by="gpu_price", ignore_index=True),
        right=expected_gpu_of_interest_df,
    )
    pdt.assert_frame_equal(
        left=pd.read_sql(
            sql="SELECT * FROM lowest_prices", con=test_db_conn
        ).sort_values(by="gpu_price", ignore_index=True),
        right=expected_lowest_prices_df,
    )
    pdt.assert_frame_equal(
        left=pd.read_sql(
            sql="SELECT * FROM lowest_prices_tiered", con=test_db_conn
        ).sort_values(by="gpu_price", ignore_index=True),
