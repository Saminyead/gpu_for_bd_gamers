...
def test_no_empty_str_read_gpu_from_files(filename: str = FILE_FOR_TEST) -> None:
...
def test_add_gpu_unit_name_default(
    ...
    geforce_df = add_gpu_unit_name(
        master_df_test_with_space, geforce_gpu_list, "Geforce"
    )
    ...
    for gpu_unit_name, index_list in gpu_unit_index["Radeon"].items():
        for i in index_list:
            ...
            assert (
                radeon_df["gpu_unit_name"][radeon_df["gpu_name"] == gpu_name].iloc[0]
                == gpu_unit_name
            )
    ...
    for gpu_unit_name, index_list in gpu_unit_index["Geforce"].items():
        for i in index_list:
            gpu_name: str = master_df_test_with_space.gpu_name.iloc[i]
            assert (
                geforce_df["gpu_unit_name"][geforce_df["gpu_name"] == gpu_name].iloc[0]
                == gpu_unit_name
            )
    ...
    for gpu_unit_name, index_list in gpu_unit_index["Intel"].items():
        for i in index_list:
            gpu_name: str = master_df_test_with_space.gpu_name.iloc[i]
            assert (
                intel_df["gpu_unit_name"][intel_df["gpu_name"] == gpu_name].iloc[0]
                == gpu_unit_name
            )
...
def test_add_gpu_unit_name_1050_ti() -> None:
    """Test to observe if GTX 1050 Ti can be detected using add_gpu_unit_name"""
    master_df = pd.DataFrame(
        data={
            "gpu_name": ["Zotac Twin Edge Geforce GTX 1050 Ti", "Uncles GTX 1050Ti"],
            "gpu_price": [13000, 14000],
            "retail_url": ["https://weridass.com/zotac-te-1050-ti", "https://uncle.com/1050-ti"]
        }
    )
    df_gpu_unit_name_added = add_gpu_unit_name(
        target_df=master_df, gpu_list=["GTX 1050 Ti"], gpu_brand="Geforce"
    )
    assert df_gpu_unit_name_added["gpu_unit_name"].iloc[0] == "Geforce GTX 1050 Ti"
